...
class AutoPersonaImageProcessor:
        ...
        for folder in self.source_dir.iterdir():
            if folder.is_dir():
                ...
                if images:
                    ...
                    print(f"No images found in: {persona_name}")
    ...
    def get_firebase_personas(self) -> Set[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"Failed to get Firebase personas: {result.stderr}")
                return set()
                
        except Exception as e:
            print(f"Error getting Firebase personas: {e}")
            return set()
    ...
    def clear_firebase_persona_images(self, persona_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"    Cleared images for: {persona_name}")
                return True
            else:
    ...
    def process_all_personas(self, force_update: bool = False) -> Dict[str, any]:
        ...
        print(f"\nProcessing personas with images:")
        print("-" * 40)
        ...
        if personas_without_folders:
            print(f"\nClearing personas without folders:")
            print("-" * 40)
            
            for persona_name in personas_without_folders:
                print(f"Clearing: {persona_name}")
        ...
        print(f"   Cleared: {len(results['cleared'])}")
