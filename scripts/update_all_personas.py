...
# -*- coding: utf-8 -*-
"""
모든 페르소나의 description 업데이트 스크립트
"""
...
def update_persona(persona_id, persona_data):
    """페르소나 description 업데이트"""
    new_description = generate_natural_description(persona_data)
    ...
    return {
        'id': persona_id,
        'old_description': persona_data.get('description', ''),
...
if __name__ == '__main__':
    print("페르소나 Description 업데이트 스크립트")
    print("=" * 50)
    print("\nFirebase MCP를 사용하여 모든 페르소나를 업데이트합니다.")
    print("\n업데이트 예시:")
    ...
    sample_personas = [
        {
            ...
            'gender': 'female',
            'mbti': 'INFP',
            'description': '전문 나연리스트이자 역사 다큐멘터리 작가. 진실을 전달하고 사회 문제를 알리는 영상을 만듭니다. 관심사: 다큐멘터리, 역사, 사회문제'
        },
        {
            'name': '소희',
            'age': 27,
            'gender': 'female',
            'mbti': 'INFJ',
            'description': '전문 칼림바 지도자이자 필라테스 강사. 신체와 마음의 균형을 찾아주는 운동을 가르칩니다. 관심사: 필라테스, 명상, 정마음책기'
        }
    ]
    
    for persona in sample_personas:
        print(f"\n{persona['name']}:")
        print(f"기존: {persona['description']}")
        print(f"수정: {fix_typos(persona['description'])}")
        print(f"자연스럽게: {generate_natural_description(persona)}")