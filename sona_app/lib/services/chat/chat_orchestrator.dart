...
class ChatOrchestrator {
  ...
  Future<ChatResponse> generateResponse({
    ...
    try {
      ...
      final speechPattern =
          UserSpeechPatternAnalyzer.analyzeSpeechPattern(userMessages);
  ...
  String? _generateSimpleTranslation(String koreanText, String targetLanguage) {
    ...
    return translationPendingMessages[targetLanguage] ??
  ...
  String _generateFallbackResponse(Persona persona) {
    ...
    final isCasualSpeech =
        false; // Fallback always uses formal speech for safety
  ...
  MessageAnalysis _analyzeUserMessage(String message) {
    ...
    else if (_isFarewell(lower))
      type = MessageType.farewell;
    else if (_isCompliment(lower))
  ...
  Future<String?> _checkSimpleResponse({
    ...
    if (_isThanks(lowerMessage)) {
      return _getThanksResponse(mbti, gender);
    }
  ...
  bool _isEnglishGreeting(String message) {
    // 영어 인사 패턴 감지
    return RegExp(r'(hi|hello|hey|how\s+(are\s+you|r\s+u))',
            caseSensitive: false)
        .hasMatch(message);
  }
  ...
  String? _detectSpecificLanguage(String message) {
    ...
    if (!hasKorean) {
      // _detectForeignLanguageQuestion 메서드로 영어 감지
      if (_detectForeignLanguageQuestion(message)) {
        debugPrint('🌍 English detected via foreign language detection');
        return 'en';
      }
      
      // 언어별 특징적인 패턴과 문자 확인
      // 영어 (영어 알파벳과 일반적인 기호만 있는 경우)
      if (RegExp(r'^[a-zA-Z0-9\s\?\.\!\,\x27\-]+$').hasMatch(message)) {
        debugPrint('🌍 English detected via alphabet pattern');
        return 'en';
      }
    } else {
      // 한국어가 있지만 영어가 대부분인 경우 체크
      final koreanPattern = RegExp(r'[가-힣ㄱ-ㅎㅏ-ㅣ]');
      final englishPattern = RegExp(r'[a-zA-Z]');
      final koreanCount = koreanPattern.allMatches(message).length;
      final englishCount = englishPattern.allMatches(message).length;
      
      // 영어가 한국어보다 2배 이상 많으면 영어로 간주
      if (englishCount > koreanCount * 2) {
        debugPrint('🌍 English detected (mixed with Korean but English dominant)');
        return 'en';
      }
  ...
  String _getGreetingResponse(String mbti, [String gender = 'female']) {
    final responses = _getPersonaResponses(mbti, 'greeting', gender);
  ...
  String _getThanksResponse(String mbti, [String gender = 'female']) {
    final responses = _getPersonaResponses(mbti, 'thanks', gender);
    // 더 나은 랜덤성을 위해 Random 사용
    final random = math.Random();
    return responses[random.nextInt(responses.length)];
  }
  ...
  String? _analyzeContextRelevance({
    ...
    if (lastAIMessage != null && lastUserMessage != null) {
      ...
      if (!hasTopicConnection &&
          ...
          !_isGreeting(userMessage.toLowerCase())) {
  ...
  bool _detectForeignLanguageQuestion(String message) {
    ...
    final clearForeignSentences = [
      ...
      RegExp(r'(what|where|when|who|why|how)\s+(r\s+|are\s+)?\w+', caseSensitive: false), // how r u 같은 축약형 포함
      // 추가 영어 축약형 패턴
