...
class UserSpeechPatternAnalyzer {
  ...
  static SpeechPattern analyzeSpeechPattern(List<String> userMessages) {
    ...
    // 9. 특징적인 표현들
    final characteristicExpressions = _extractCharacteristicExpressions(recentMessages);
  ...
  static EmoticonPattern _analyzeEmoticonUsage(List<String> messages) {
    ...
    for (final msg in messages) {
      ...
      // 이모지 카운트
      final emojiMatches = RegExp(r'[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]', unicode: true).allMatches(msg);
      ...
      final textEmoticonPatterns = [
        r'\^\^', r'><', r'TT', r'ㅠㅠ', r'ㅜㅜ', r'-_-', r';;',
        r'\*\.\*', r'@_@', r'o_o', r'O_O', r'>_<', r'\+_\+',
      ];
  ...
  static LaughPattern _analyzeLaughPattern(List<String> messages) {
    ...
    if ((laughTypes['short_k'] ?? 0) + (laughTypes['medium_k'] ?? 0) > maxCount) {
      primaryType = 'ㅋㅋ';
  ...
  static double _analyzeAbbreviationUsage(List<String> messages) {
    final abbreviations = [
      'ㅇㅇ', 'ㄴㄴ', 'ㅇㅋ', 'ㄱㅅ', 'ㅈㅅ', 'ㅊㅋ', 'ㅅㄱ', 'ㅎㅇ', 'ㅂㅂ', 'ㅂㅇ',
      'ㄹㅇ', 'ㅇㅈ', 'ㄱㄱ', 'ㄷㄷ', 'ㅎㄷㄷ', 'ㅈㅈ', 'ㅅㅂ', 'ㅁㅊ',
      '갑분싸', '별다줄', '케바케', '오운완', 'TMI', 'JMT', '존맛', '개맛', '노맛',
      '혼밥', '혼술', '혼영', '넷플', '인스타', '카톡', '디코',
      '저메추', '점메추', '아메추', '야메추', '김찌', '된찌', '제육', '김볶',
      '치맥', '피맥', '소맥', '막소', '월요병', '불금', '칼퇴',
    ];
  ...
  static QuestionStyle _analyzeQuestionStyle(List<String> messages) {
    ...
    final directness = questions.isNotEmpty ? directCount / questions.length : 0.5;
  ...
  static String generateAdaptationGuide(SpeechPattern pattern, String personaGender) {
    ...
    if (pattern.laughPattern.intensity > 1.5) {
      buffer.writeln('  (자주 웃음 - 대화의 ${(pattern.laughPattern.intensity * 100).round()}%에서 사용)');
    }
    
    // 3. 이모티콘
    if (pattern.emoticonPattern.frequency > 0.3) {
      buffer.writeln('- 이모티콘 적극 사용 (${pattern.emoticonPattern.preferEmoji ? "이모지" : "텍스트 이모티콘"} 선호)');
      if (pattern.emoticonPattern.commonEmojis.isNotEmpty) {
        buffer.writeln('  자주 쓰는 이모지: ${pattern.emoticonPattern.commonEmojis.join(", ")}');
      }
    }
    ...
    if (pattern.characteristicExpressions.isNotEmpty) {
      buffer.writeln('- 사용자가 자주 쓰는 표현: ${pattern.characteristicExpressions.take(3).join(", ")}');
...
/// 문장 끝맺음 스타일
class EndingStyle {
  final String primaryEnding; // none, exclamation, question, wave, ellipsis, period
