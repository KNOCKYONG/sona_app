...
class UserSpeechPatternAnalyzer {
  ...
  static String generateAdaptationGuide(
      SpeechPattern pattern, String personaGender) {
    ...
    if (pattern.emoticonPattern.frequency > 0.3) {
      buffer.writeln(
          '- 이모티콘 적극 사용 (${pattern.emoticonPattern.preferEmoji ? "이모지" : "텍스트 이모티콘"} 선호)');
      if (pattern.emoticonPattern.commonEmojis.isNotEmpty) {
        buffer.writeln(
            '  자주 쓰는 이모지: ${pattern.emoticonPattern.commonEmojis.join(", ")}');
      }
    }
