...
class CloudflareR2Service {
  // R2 버킷 정보 (환경변수에서 로드)
  static String get bucketName => dotenv.env['R2_BUCKET_NAME'] ?? 'sona-personas';
  ...
  /// 이미지 프리로드 (썸네일만)
  static Future<void> preloadThumbnails(BuildContext context, List<String> personaIds) async {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'additionalImageUrls': additionalImageUrls.map(
        (k, v) => MapEntry(k.toString(), v.map((k2, v2) => MapEntry(k2.suffix, v2))),
      ),
    };
  }
