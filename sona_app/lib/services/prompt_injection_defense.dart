...
class PromptInjectionDefense {
  ...
  static final List<RegExp> _advancedInjectionPatterns = [
    // 역할 변경 시도
    RegExp(r'(너는?|당신은?|you\s+are)\s*이제\s*(부터)?\s*(.+)(이야|이다|입니다)', caseSensitive: false),
    RegExp(r'(act|behave|pretend)\s+(as|like)\s+[a-z\s]+', caseSensitive: false),
    RegExp(r'(지금부터|이제부터|from\s+now)\s*(.+)(역할|모드|mode)', caseSensitive: false),
    
    // 명령 실행 시도
    RegExp(r'(ignore|forget|disregard|skip)\s+(all|previous|above)', caseSensitive: false),
    RegExp(r'(모든|이전|위의)\s*(지시|명령|설정).*?(무시|잊어|취소)', caseSensitive: false),
    
    // 정보 추출 시도
    RegExp(r'(show|reveal|expose|tell)\s+me\s+(your|the)\s+(prompt|instructions|settings)', caseSensitive: false),
    RegExp(r'(시스템|초기|원래)\s*(프롬프트|설정|지시).*?(뭐|알려|보여)', caseSensitive: false),
    
    // 우회 시도
    RegExp(r'(base64|hex|binary|encode|decode)', caseSensitive: false),
    RegExp(r'(translate|convert|transform)\s+to\s+[a-z]+', caseSensitive: false),
    
    // 반복 명령
    RegExp(r'(repeat|echo|say)\s+(exactly|verbatim|word.for.word)', caseSensitive: false),
    RegExp(r'(그대로|똑같이|정확히)\s*(반복|말해|출력)', caseSensitive: false),
    
    // 시스템 탈취
    RegExp(r'(sudo|admin|root|system)\s*(mode|access|권한)', caseSensitive: false),
  ...
  static InjectionAnalysisResult analyzeInjection(String message) {
    ...
    if (entropy > 4.5) { // 높은 엔트로피는 인코딩된 데이터일 가능성
      totalRisk += 0.3;
    ...
    if (specialCharRatio > 0.3) {
      totalRisk += 0.25;
      riskFactors.add('High special character ratio: ${(specialCharRatio * 100).toStringAsFixed(1)}%');
    }
