...
class CacheManager {
  ...
  Future<void> markTutorialCompleted() async {
    try {
      ...
      await prefs.setInt(
          'tutorial_completed_at', DateTime.now().millisecondsSinceEpoch);
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<bool> _isImageCacheExpired(String hash) async {
    try {
      ...
      final cachedAt =
          DateTime.fromMillisecondsSinceEpoch(metadata['cachedAt']);
  ...
  Future<void> _cleanImageCacheBySize() async {
    try {
      ...
      final keys = prefs
          .getKeys()
          .where((key) => key.startsWith('cache_image_'))
          .toList();
